...
// In-memory session id, used when sessionStorage is unavailable (private mode,
// a WebView with storage blocked). Without it the catch below minted a NEW uuid
// on every read, so every single event of one session carried a different
// session_id and no session could ever be reconstructed.
...
export function startAnalyticsSession(): string {
  ...
  try {
    ...
    /* ignore — the in-memory id above still keeps the session coherent */
  }
  return id;
}
