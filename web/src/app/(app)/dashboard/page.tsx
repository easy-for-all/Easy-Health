  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Personal Records */}
        <PersonalRecordsBlock records={personalRecords.slice(0, 3)} />
...
// daysAgo replaced by relativeDayLabel from @/shared/utils/relative-date
    ...
    <div style={{ borderRadius: "var(--r-lg)", background: "var(--surface)", border: "1px solid var(--border)", padding: "16px" }}>
      ...
      {records.length === 0 ? (
        <p style={{ fontSize: 13, color: "var(--text-dim)", margin: 0, lineHeight: 1.5 }}>
          Conclua treinos com cargas para ver seus recordes pessoais aqui.
        </p>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {records.map((pr) => {
            const newToday = isToday(pr.achieved_at);
            return (
              <div key={pr.exercise_id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                    ...
                    {newToday && (
                      <span style={{
                        fontSize: 10, fontWeight: 700, color: "oklch(0.85 0.18 95)",
                        background: "oklch(0.85 0.18 95 / 0.12)", borderRadius: 4,
                  ...
                  <p style={{ fontSize: 11, color: "var(--text-dim)", margin: "2px 0 0" }}>{relativeDayLabel(pr.achieved_at)}</p>
                </div>
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  ...
                  <p style={{ fontSize: 10, color: "var(--text-dim)", margin: "2px 0 0" }}>Melhor carga</p>
                </div>
              </div>
            );
          })}
...
function ModalityMetricsCard({ modality, stats }: { modality: string; stats: Record<string, number | null> }) {
  if (!stats || !stats.sessions_count) return null;
  ...
  items.push({ label: "Sessões (30 dias)", value: String(stats.sessions_count) });
