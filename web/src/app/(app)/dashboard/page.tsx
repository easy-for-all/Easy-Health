  ...
  if (noProfile) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center px-4">
        ...
        <Link href="/onboarding" className="rounded-lg bg-green-500 px-6 py-3 text-sm font-semibold text-white hover:bg-green-600">
          Completar perfil
        </Link>
      </div>
    );
  ...
  return (
    <Link href="/workout/today">
      <div className="rounded-2xl bg-green-500 p-5 text-white">
        <p className="text-sm font-medium text-green-100">Treinar agora</p>
        <p className="mt-1 text-2xl font-bold">Escolha A, B, C...</p>
        <p className="mt-1 text-sm text-green-100">{plan?.days?.length ?? 0} treinos disponíveis</p>
        <div className="mt-4 inline-block rounded-full bg-white px-4 py-2 text-sm font-semibold text-green-600">
          Escolher treino →
        </div>
      </div>
    </Link>
  ...
  return (
    <div className="rounded-xl border border-gray-100 bg-white p-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs font-semibold text-green-600">Treino {LETTERS[index] ?? index + 1}</p>
          <p className="font-semibold text-gray-900">{day.name}</p>
          <p className="text-xs text-gray-500">{day.exercise_count} exercícios</p>
        </div>
        <Link href="/workout/today" className="rounded-full bg-green-500 px-3 py-1.5 text-xs font-semibold text-white">
          Treinar
        </Link>
      </div>
    </div>
